...
class TestEndpoint(unittest.TestCase):
    def test_endpoint(self):
        usermod = "admin"
        username = "adminTestJson"
        passw = "adminTestJson123"
        usermodUrl = baseUrl + f"admin/{usermod}/{username}/{passw}" + "?format=json"
        vescookie = getCookie()
        response = requests.post(url = usermodUrl, verify = False, cookies = vescookie)
...
if __name__ == "__main__":
    ...
    print(f"Usermod Unit Test Result:\t\t\t{result}")