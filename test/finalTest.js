...
describe("Super-Admin Operations good scenario 1 (healthcheck, resetAll, restq)", () => {
    ...
    describe("/admin/resetall", () => {
        it("it should delete all users(except super-admin), answers, options, questions, questionnaires, sessions", (done) => {
          chai
            .request(server)
...
//------------------------------------------------Good Senario----------------------------------//
//--------------------------------Recreate testAdmin and testQuestionnaire to use for testing----------//
    ...
    describe("/admin/resetq/:questionnaireID", () => {
      ...
      describe("/admin/resetall", () => {
        it("it should throw an error because format != json && format = csv", (done) => {
          chai
            .request(server)
            .post("/intelliq_api/admin/resetall?format=jso")
            .set("Cookie", `jwt=${token}`)
            .end((err, res) => {
                res.should.have.status(400);
                res.body.should.have.property("status");
